template<typename T>
DoryCOT<T>::DoryCOT(int party, int threads, T **ios,
...
template<typename T>
void DoryCOT<T>::extend_f2k(block *ot_buffer) {
	if(party == ALICE)
...
// extend f2k
template<typename T>
void DoryCOT<T>::extend_f2k() {
	extend_f2k(ot_data);
}

template<typename T>
void DoryCOT<T>::setup(block Deltain, std::string pre_file, bool *choice, block seed) {
	this->Delta = Deltain;
...
template<typename T>
void DoryCOT<T>::setup(std::string pre_file, bool *choice, block seed) {
...
template<typename T>
void DoryCOT<T>::rcot(block *data, int64_t num) {
	if(ot_data == nullptr) {
...
template<typename T>
int64_t DoryCOT<T>::silent_ot_left() {
	return ot_limit-ot_used;
}

template<typename T>
void DoryCOT<T>::write_pre_data128_to_file(void* loc, __uint128_t delta, std::string filename) {
...
template<typename T>
__uint128_t DoryCOT<T>::read_pre_data128_from_file(void* pre_loc, std::string filename) {
...
template<typename T>
int64_t DoryCOT<T>::byte_memory_need_inplace(int64_t ot_need) {
...
template<typename T>
int64_t DoryCOT<T>::rcot_inplace(block *ot_buffer, int64_t byte_space, block seed) {
...
template<typename T>
void DoryCOT<T>::online_sender(block *data, int64_t length) {
...
template<typename T>
void DoryCOT<T>::online_recver(block *data, const bool *b, int64_t length) {
	bool *bo = new bool[length];
	for(int64_t i = 0; i < length; ++i) {
...
template<typename T>
void DoryCOT<T>::send_cot(block * data, int64_t length) {
	rcot(data, length);
	online_sender(data, length);
}

template<typename T>
void DoryCOT<T>::recv_cot(block* data, const bool * b, int64_t length) {
	rcot(data, length);
